...
interface CipherState {
    ...
    fun initializeKey(key: ByteArray): CipherState = apply(key, cipher())
    ...
    companion object {
        ...
        fun apply(cipher: CipherFunctions): CipherState = UninitializedCipherState(cipher)
    }
}
 ...
 */
data class UninitializedCipherState(val cipher: CipherFunctions) : CipherState {
 ...
 * @param cipher cipher functions
 */
data class InitializedCipherState(val k: ByteArray, val n: Long, val cipher: CipherFunctions) : CipherState {
    init {
...
data class SymmetricState(val cipherState: CipherState, val ck: ByteArray, val h: ByteArray, val hashFunctions: HashFunctions) {
    ...
    companion object {
        fun apply(protocolName: ByteArray, cipherFunctions: CipherFunctions, hashFunctions: HashFunctions): SymmetricState {
            ...
            return SymmetricState(CipherState.apply(cipherFunctions), ck = h, h = h, hashFunctions = hashFunctions)
        }
    }
}
...
data class HandshakePattern(val name: String, val initiatorPreMessages: List<MessagePattern>, val responderPreMessages: List<MessagePattern>, val messages: List<List<MessagePattern>>) {
    init {
        require(isValidInitiator(initiatorPreMessages)) { "invalid initiator messages" }
        require(isValidInitiator(responderPreMessages)) { "invalid responder messages" }
    }

    companion object {
        val validInitiatorPatterns: Set<List<MessagePattern>> = setOf(listOf<MessagePattern>(), listOf(MessagePattern.E), listOf(MessagePattern.S), listOf(MessagePattern.E, MessagePattern.S))
...
val handshakePatternNN = HandshakePattern("NN", initiatorPreMessages = listOf<MessagePattern>(), responderPreMessages = listOf<MessagePattern>(), messages = listOf(listOf(MessagePattern.E), listOf(MessagePattern.E, MessagePattern.EE)))
val handshakePatternXK = HandshakePattern("XK", initiatorPreMessages = listOf<MessagePattern>(), responderPreMessages = listOf(MessagePattern.S), messages = listOf(listOf(MessagePattern.E, MessagePattern.ES), listOf(MessagePattern.E, MessagePattern.EE), listOf(MessagePattern.S, MessagePattern.SE)))
...
sealed class HandshakeState {
    ...
    companion object {
        private fun makeSymmetricState(handshakePattern: HandshakePattern, prologue: ByteArray, dh: DHFunctions, cipher: CipherFunctions, hash: HashFunctions): SymmetricState {
            ...
            val symmetricState = SymmetricState.apply(name.encodeToByteArray(), cipher, hash)
        ...
        fun initializeWriter(handshakePattern: HandshakePattern, prologue: ByteArray, s: Pair<ByteArray, ByteArray>, e: Pair<ByteArray, ByteArray>, rs: ByteArray, re: ByteArray, dh: DHFunctions, cipher: CipherFunctions, hash: HashFunctions, byteStream: ByteStream = RandomBytes): HandshakeStateWriter {
            ...
            return HandshakeStateWriter(handshakePattern.messages, symmetricState2, s, e, rs, re, dh, byteStream)
        }
        ...
        fun initializeReader(handshakePattern: HandshakePattern, prologue: ByteArray, s: Pair<ByteArray, ByteArray>, e: Pair<ByteArray, ByteArray>, rs: ByteArray, re: ByteArray, dh: DHFunctions, cipher: CipherFunctions, hash: HashFunctions, byteStream: ByteStream = RandomBytes): HandshakeStateReader {
            ...
            return HandshakeStateReader(handshakePattern.messages, symmetricState2, s, e, rs, re, dh, byteStream)
        }
...
data class HandshakeStateWriter(val messages: List<List<MessagePattern>>, val state: SymmetricState, val s: Pair<ByteArray, ByteArray>, val e: Pair<ByteArray, ByteArray>, val rs: ByteArray, val re: ByteArray, val dh: DHFunctions, val byteStream: ByteStream) : HandshakeState() {
    ...
    fun toReader(): HandshakeStateReader = HandshakeStateReader(messages, state, s, e, rs, re, dh, byteStream)
    ...
    companion object {
        fun apply(messages: List<List<MessagePattern>>, state: SymmetricState, s: Pair<ByteArray, ByteArray>, e: Pair<ByteArray, ByteArray>, rs: ByteArray, re: ByteArray, dh: DHFunctions): HandshakeStateWriter = HandshakeStateWriter(messages, state, s, e, rs, re, dh, RandomBytes)
    }
}
...
data class HandshakeStateReader(val messages: List<List<MessagePattern>>, val state: SymmetricState, val s: Pair<ByteArray, ByteArray>, val e: Pair<ByteArray, ByteArray>, val rs: ByteArray, val re: ByteArray, val dh: DHFunctions, val byteStream: ByteStream) : HandshakeState() {
    fun toWriter(): HandshakeStateWriter = HandshakeStateWriter(messages, state, s, e, rs, re, dh, byteStream)
    ...
    companion object {
        ...
        fun apply(messages: List<List<MessagePattern>>, state: SymmetricState, s: Pair<ByteArray, ByteArray>, e: Pair<ByteArray, ByteArray>, rs: ByteArray, re: ByteArray, dh: DHFunctions): HandshakeStateReader = HandshakeStateReader(messages, state, s, e, rs, re, dh, RandomBytes)
    }
}