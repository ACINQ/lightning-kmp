...
class ElectrumClientTest : LightningTestSuite() {
    // this is tx #2 of block #130003 on testnet4
    ...
    private val scriptPubkey = referenceTx.txOut.first().publicKeyScript
    private val scriptHash = Crypto.sha256(scriptPubkey).toByteVector32().reversed()
    ...
    @Test
    fun `scriptpubkey subscription`() = runTest { client ->
    ...
    @Ignore // removed in protocol 1.7
    ...
    @Test
    fun `get scriptpubkey history`() = runTest { client ->
        val history = client.getScriptPubkeyHistory(scriptPubkey)
        assertTrue(history.contains(TransactionHistoryItem(130_003, referenceTx.txid)))
        client.stop()
    }

    @Test
    fun `list scriptHash unspents`() = runTest { client ->
    ...
    @Test
    fun `list scriptPubkey unspents`() = runTest { client ->
        val response = client.getScriptPubkeyUnspents(scriptPubkey)
        assertTrue(response.isEmpty())
        client.stop()
    }
