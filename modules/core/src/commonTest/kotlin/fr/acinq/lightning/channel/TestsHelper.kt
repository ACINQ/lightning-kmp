    ...
    private fun checkSerialization(state: PersistedChannelState) {
        ...
        fun removeTemporaryStatuses(state: PersistedChannelState): PersistedChannelState = when (state) {
            is WaitForFundingConfirmed -> when (state.rbfStatus) {
                is RbfStatus.WaitingForSigs -> state.updateCommitments(state.commitments.copy(remoteCommitNonces = mapOf()))
                else -> state.copy(rbfStatus = RbfStatus.None).updateCommitments(state.commitments.copy(remoteCommitNonces = mapOf()))
            }
            ...
            is WaitForFundingSigned -> state.copy(signingSession = state.signingSession.copy(nextRemoteNonce = null))
            is ChannelStateWithCommitments -> {
                state.updateCommitments(state.commitments.copy(remoteCommitNonces = mapOf()))
            }
        }
        ...
        fun ignoreClosingReplyTo(state: PersistedChannelState): PersistedChannelState = when (state) {
            is Normal -> state.copy(closeCommand = state.closeCommand?.copy(replyTo = dummyReplyTo)).updateCommitments(state.commitments.copy(remoteCommitNonces = mapOf()))
