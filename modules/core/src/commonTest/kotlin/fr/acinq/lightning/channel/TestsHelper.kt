...
internal fun List<ChannelAction>.hasWatchOutputSpent(outpoint: OutPoint): WatchSpent = hasWatch<WatchSpent>().also { assertEquals(outpoint.txid, it.txId); assertEquals(outpoint.index, it.outputIndex.toLong()); assertIs<WatchSpent.ClosingOutputSpent>(it.event) }
...
object TestsHelper {
    ...
    fun localClose(s: LNChannel<ChannelState>, htlcSuccessCount: Int = 0, htlcTimeoutCount: Int = 0): Pair<LNChannel<Closing>, LocalCommitPublished> {
        ...
        return closingState to localCommitPublished
    }

    fun remoteClose(rCommitTx: Transaction, s: LNChannel<ChannelState>, htlcSuccessCount: Int = 0, htlcTimeoutCount: Int = 0): Pair<LNChannel<Closing>, RemoteCommitPublished> {
        assertIs<LNChannel<ChannelStateWithCommitments>>(s)
        assertEquals(Transactions.CommitmentFormat.AnchorOutputs, s.state.commitments.latest.commitmentFormat)
        ...
        return closingState to remoteCommitPublished
    }
