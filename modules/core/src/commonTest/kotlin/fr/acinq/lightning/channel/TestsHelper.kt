...
object TestsHelper {
    ...
    fun remoteClose(rCommitTx: Transaction, s: LNChannel<ChannelState>): Pair<LNChannel<Closing>, RemoteCommitPublished> {
        ...
        remoteCommitPublished.claimHtlcTxs.values.filterNotNull().forEach { claimHtlc ->
            val result = kotlin.runCatching { Transaction.correctlySpends(claimHtlc.tx, rCommitTx, ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS) }
            if (result.isFailure) {
                println("htlc tx ${claimHtlc.tx.txid} is bad!!")
            }
    ...
    fun useAlternativeCommitSig(s: LNChannel<ChannelState>, commitment: Commitment, alternative: Either<CommitSigTlv.AlternativeFeerateSig, CommitSigTlv.AlternativeFeeratePartialSig>): Transaction {
        ...
        return when (alternative) {
            is Either.Left -> {
                val localSig = localCommitTx.sign(channelKeys.fundingKey(fundingTxIndex))
