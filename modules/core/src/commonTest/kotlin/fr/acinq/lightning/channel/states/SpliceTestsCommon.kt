...
class SpliceTestsCommon : LightningTestSuite() {
    ...
    fun `splice to purchase inbound liquidity`() {
        ...
        val fundingScript = Transactions.makeFundingScript(spliceInit.fundingPubkey, defaultSpliceAck.fundingPubkey, Transactions.CommitmentFormat.AnchorOutputs).pubkeyScript
        run {
    ...
    fun `force-close -- revoked previous active commitment`() {
        ...
        assertEquals(htlcs.aliceToBob.size + htlcs.bobToAlice.size, rvk.htlcOutputs.size)
        assertEquals(rvk.htlcOutputs, htlcPenaltyTxs.flatMap { tx -> tx.txIn.map { it.outPoint } }.toSet())
        htlcPenaltyTxs.forEach { Transaction.correctlySpends(it, bobRevokedCommitTx, ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS) }
        actionsAlice11.hasWatchOutputsSpent(rvk.htlcOutputs)
