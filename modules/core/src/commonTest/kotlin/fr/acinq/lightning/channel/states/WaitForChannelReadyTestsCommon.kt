...
class WaitForChannelReadyTestsCommon : LightningTestSuite() {
    ...
    fun `recv ChannelReady -- after restart`() {
        val (alice, _, bob, channelReadyBob) = init(bobFeatures = TestConstants.Bob.nodeParams.features.remove(Feature.ChannelBackupClient).initFeatures())
    ...
    companion object {
        ...
        fun init(
            ...
            return if (zeroConf) {
                val (alice, commitAlice, bob, commitBob) = WaitForFundingSignedTestsCommon.init(channelType, aliceFeatures, bobFeatures, currentHeight, aliceFundingAmount, bobFundingAmount, requestRemoteFunding, zeroConf)
