...
class ElectrumRequestTest : LightningTestSuite() {
    ...
    fun `ServerVersion stringify JSON-RPC format`() {
        assertEquals(buildString {
            ...
            appendLine()
        }, ServerVersion(ElectrumClient.ELECTRUM_CLIENT_NAME, "1.4", "1.4").asJsonRPCRequest(0))

        assertEquals(buildString {
            append("""{"jsonrpc":"2.0","id":0,"method":"server.version","params":["3.3.6",["1.4","1.7"]]}""")
            appendLine()
        }, ServerVersion(ElectrumClient.ELECTRUM_CLIENT_NAME, "1.4", "1.7").asJsonRPCRequest(0))

        assertEquals(buildString {
            append("""{"jsonrpc":"2.0","id":0,"method":"server.version","params":["lightning-kmp-test","1.4.2"]}""")
            appendLine()
        }, ServerVersion(clientName = "lightning-kmp-test", protocolMinVersion = "1.4.2", protocolMaxVersion = "1.4.2").asJsonRPCRequest(0))
    }
