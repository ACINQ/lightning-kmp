        ...
        return state.run { ctx.process(cmd) }
            .let { (state1, actions) ->
                if (nodeParams.usePeerStorage &&
                    ...
                    val persistedChannelStates = (channels + (state1.channelId to state1)).values.filterIsInstance<PersistedChannelState>()
                    Pair(state1, actions + ChannelAction.Message.Send(PeerStorageStore(EncryptedPeerStorage.from(nodeParams.nodePrivateKey, persistedChannelStates, logger))))
                } else {
                    Pair(state1, actions)
                }
            }
                        ...
                        if (nodeParams.usePeerStorage) {
                            ...
                            when (backup) {
                                ...
                                is PeerStorageDeserializationResult.UnknownVersion -> {
                                    logger.warning { "peer sent a peer storage retrieval with a backup generated by a more recent of phoenix: version=${backup.version}." }
