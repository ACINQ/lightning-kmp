        ...
        return when (cmd) {
            ...
            is ChannelCommand.Closing.GetHtlcInfosResponse -> {
                ...
                if (index >= 0) {
                    ...
                    val dustLimit = commitments.latest.localCommitParams.dustLimit
                    val htlcPenaltyTxs =
                        Closing.RevokedClose.run { claimRevokedRemoteCommitTxHtlcOutputs(commitments.channelParams, channelKeys(), revokedCommitPublished[index], dustLimit, commitmentFormat, currentOnChainFeerates(), cmd.htlcInfos) }
