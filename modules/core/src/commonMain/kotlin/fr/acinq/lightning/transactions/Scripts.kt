...
object Scripts {
    ...
    object Taproot {
         ...
         */
        fun witnessSpendsLeftBranch(witness: ScriptWitness, scriptTree: ScriptTree): Boolean {
            return when (scriptTree) {
                is ScriptTree.Branch -> when (scriptTree.left) {
                    is ScriptTree.Leaf -> witness.stack.size >= 3 && witness.stack[witness.stack.size - 2] == (scriptTree.left as ScriptTree.Leaf).script
                    else -> false
                }
