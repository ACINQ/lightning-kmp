...
data class TxComplete(
    ...
    val sharedInputNonces: List<IndividualNonce> = tlvs.get<TxCompleteTlv.SharedInputNonces>()?.nonces ?: listOf()

    constructor(channelId: ByteVector32, swapInNonces: List<IndividualNonce>, sharedInputNonces: List<IndividualNonce>) : this(channelId, TlvStream(TxCompleteTlv.SwapInNonces(swapInNonces), TxCompleteTlv.SharedInputNonces(sharedInputNonces)))
    ...
    companion object : LightningMessageReader<TxComplete> {
        ...
        val readers = mapOf(
            ...
            TxCompleteTlv.SharedInputNonces.tag to TxCompleteTlv.SharedInputNonces.Companion as TlvValueReader<TxCompleteTlv>
        )
...
data class SpliceInit(
    ...
    constructor(channelId: ByteVector32, fundingContribution: Satoshi, feerate: FeeratePerKw, lockTime: Long, fundingPubkey: PublicKey, requestFunding: LiquidityAds.RequestFunding?, nextLocalNonces: List<IndividualNonce> = listOf()) : this(
        channelId,
        fundingContribution,
        feerate,
        lockTime,
        ...
        TlvStream(
            setOfNotNull(
                ...
                if (nextLocalNonces.isNotEmpty()) ChannelTlv.NextLocalNoncesTlv(nextLocalNonces) else null
            )
        )
    )
...
data class SpliceAck(
    ...
    constructor(channelId: ByteVector32, fundingContribution: Satoshi, fundingPubkey: PublicKey, willFund: LiquidityAds.WillFund?, nextLocalNonces: List<IndividualNonce> = listOf()) : this(
        channelId,
        fundingContribution,
        fundingPubkey,
        TlvStream(
