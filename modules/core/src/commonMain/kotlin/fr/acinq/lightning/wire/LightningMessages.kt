...
data class TxComplete(
    ...
    val fundingNonces: List<IndividualNonce> = tlvs.get<TxCompleteTlv.FundingNonces>()?.nonces ?: listOf()
    val commitNonces: List<IndividualNonce> = tlvs.get<TxCompleteTlv.CommitNonces>()?.nonces ?: listOf()

    constructor(channelId: ByteVector32, swapInNonces: List<IndividualNonce>, fundingNonces: List<IndividualNonce>, commitNonces: List<IndividualNonce>) : this(
        channelId,
        TlvStream(TxCompleteTlv.SwapInNonces(swapInNonces), TxCompleteTlv.FundingNonces(fundingNonces), TxCompleteTlv.CommitNonces(commitNonces))
    )
    ...
    companion object : LightningMessageReader<TxComplete> {
        ...
        val readers = mapOf(
            ...
            TxCompleteTlv.FundingNonces.tag to TxCompleteTlv.FundingNonces.Companion as TlvValueReader<TxCompleteTlv>,
...
data class ClosingComplete(
    ...
    val closerOutputOnlyPartialSig: ByteVector32? = tlvStream.get<ClosingCompleteTlv.CloserOutputOnlyPartialSignature>()?.partialSignature
    val closeeOutputOnlyPartialSig: ByteVector32? = tlvStream.get<ClosingCompleteTlv.CloseeOutputOnlyPartialSignature>()?.partialSignature
    val closerAndCloseeOutputsPartialSig: ByteVector32? = tlvStream.get<ClosingCompleteTlv.CloserAndCloseeOutputsPartialSignature>()?.partialSignature
...
data class ClosingSig(
    ...
    val closerOutputOnlyPartialSig: ByteVector32? = tlvStream.get<ClosingSigTlv.CloserOutputOnlyPartialSignature>()?.partialSig
    val closeeOutputOnlyPartialSig: ByteVector32? = tlvStream.get<ClosingSigTlv.CloseeOutputOnlyPartialSignature>()?.partialSig
    val closerAndCloseeOutputsPartialSig: ByteVector32? = tlvStream.get<ClosingSigTlv.CloserAndCloseeOutputsPartialSignature>()?.partialSig
