...
sealed class ClosingCompleteTlv : Tlv {
    ...
    /** Signature for a closing transaction containing only the closer's output. */
    data class CloserOutputOnlyPartialSignature(val partialSignature: ByteVector32) : ClosingCompleteTlv() {
        override val tag: Long get() = CloserOutputOnlyPartialSignature.tag
        override fun write(out: Output) = LightningCodecs.writeBytes(partialSignature.toByteArray(), out)

        companion object : TlvValueReader<CloserOutputOnlyPartialSignature> {
            const val tag: Long = 4
            override fun read(input: Input): CloserOutputOnlyPartialSignature = CloserOutputOnlyPartialSignature(LightningCodecs.bytes(input, 32).toByteVector32())
        }
    }

    /** Signature for a closing transaction containing only the closee's output. */
    data class CloseeOutputOnlyPartialSignature(val partialSignature: ByteVector32) : ClosingCompleteTlv() {
        ...
        companion object : TlvValueReader<CloseeOutputOnlyPartialSignature> {
            ...
            override fun read(input: Input): CloseeOutputOnlyPartialSignature = CloseeOutputOnlyPartialSignature(LightningCodecs.bytes(input, 32).toByteVector32())
        }
    }

    /** Signature for a closing transaction containing the closer and closee's outputs. */
    data class CloserAndCloseeOutputsPartialSignature(val partialSignature: ByteVector32) : ClosingCompleteTlv() {
        ...
        companion object : TlvValueReader<CloserAndCloseeOutputsPartialSignature> {
            const val tag: Long = 6
            override fun read(input: Input): CloserAndCloseeOutputsPartialSignature = CloserAndCloseeOutputsPartialSignature(LightningCodecs.bytes(input, 32).toByteVector32())
        }
    }
...
sealed class ClosingSigTlv : Tlv {
    ...
    /** Signature for a closing transaction containing only the closer's output. */
    data class CloserOutputOnlyPartialSignature(val partialSig: ByteVector32) : ClosingSigTlv() {
        override val tag: Long get() = CloserOutputOnlyPartialSignature.tag
        override fun write(out: Output) = LightningCodecs.writeBytes(partialSig.toByteArray(), out)
    ...
    /** Signature for a closing transaction containing only the closee's output. */
    data class CloseeOutputOnlyPartialSignature(val partialSig: ByteVector32) : ClosingSigTlv() {
        override val tag: Long get() = CloseeOutputOnlyPartialSignature.tag
        override fun write(out: Output) = LightningCodecs.writeBytes(partialSig.toByteArray(), out)
    ...
    /** Signature for a closing transaction containing the closer and closee's outputs. */
    data class CloserAndCloseeOutputsPartialSignature(val partialSig: ByteVector32) : ClosingSigTlv() {
        override val tag: Long get() = CloserAndCloseeOutputsPartialSignature.tag
        override fun write(out: Output) = LightningCodecs.writeBytes(partialSig.toByteArray(), out)
