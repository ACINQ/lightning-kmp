...
object Transactions {
    ...
    sealed class InputInfo {
        ...
        data class TaprootInput(override val outPoint: OutPoint, override val txOut: TxOut, val internalKey: XonlyPublicKey, val scriptTree: ScriptTree?) : InputInfo() {
            override val isP2tr: Boolean get() = true
        }
    }
    ...
    fun makeClaimRemoteDelayedOutputTx(
        ...
        fun makeClaimRemoteDelayedOutputTxTaproot(): TxResult<TransactionWithInputInfo.ClaimRemoteCommitMainOutputTx.ClaimRemoteDelayedOutputTx> {
            ...
            return findPubKeyScriptIndex(commitTx, pubkeyScript).flatMap { outputIndex ->
                val input = InputInfo.TaprootInput(OutPoint(commitTx, outputIndex.toLong()), commitTx.txOut[outputIndex], NUMS_POINT.xOnly(), Taproot.toRemoteScriptTree(localPaymentPubkey))
        ...
        fun makeClaimHtlcDelayedOutputPenaltyTxsTaproot(): TxResult<List<TxResult<TransactionWithInputInfo.ClaimHtlcDelayedOutputPenaltyTx>>> {
            ...
            return findPubKeyScriptIndexes(delayedOutputTx, pubkeyScript).map { outputIndexes ->
                outputIndexes.map { outputIndex ->
                    val input = InputInfo.TaprootInput(OutPoint(delayedOutputTx, outputIndex.toLong()), delayedOutputTx.txOut[outputIndex], localRevocationPubkey.xOnly(), tree.left)
                    makeUnsignedTx(input)
                }
            }
        }
    ...
    fun makeMainPenaltyTx(
        ...
        fun makeMainPenaltyTxTaproot(): TxResult<TransactionWithInputInfo.MainPenaltyTx> {
            ...
            return findPubKeyScriptIndex(commitTx, pubkeyScript).flatMap { outputIndex ->
                val input = InputInfo.TaprootInput(OutPoint(commitTx, outputIndex.toLong()), commitTx.txOut[outputIndex], NUMS_POINT.xOnly(), tree)
