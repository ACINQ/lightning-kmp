...
object Deserialization {
    ...
    private fun Input.readLocalCommitWithHtlcs(remoteFundingPubKey: PublicKey): LocalCommit {
        val index = readNumber()
        val spec = readCommitmentSpecWithHtlcs()
        val (commitTxId, remoteSig) = readRemoteCommitSig(remoteFundingPubKey)
    ...
    private fun Input.readLocalCommitWithoutHtlcs(htlcs: Set<DirectedHtlc>, remoteFundingPubKey: PublicKey): LocalCommit {
        val index = readNumber()
        val spec = readCommitmentSpecWithoutHtlcs(htlcs)
        val (commitTxId, remoteSig) = readRemoteCommitSig(remoteFundingPubKey)
        val htlcSigs = readHtlcRemoteSigs()
    ...
    private fun Input.readRemoteCommitSig(remoteFundingPubKey: PublicKey): Pair<TxId, ByteVector64> {
        require(read() == 0x00) // legacy discriminator for commit tx
        val (_, redeemScript) = readInputInfoWithRedeemScript()
        val commitTx = readTransaction()
        val remoteSig = extractRemoteCommitSig(commitTx, redeemScript, remoteFundingPubKey)
        return Pair(commitTx.txid, remoteSig)
    }
    ...
    private fun Input.readCommitment(htlcs: Set<DirectedHtlc>, localCommitParams: CommitParams, remoteCommitParams: CommitParams): Commitment {
        ...
        val localFundingStatus = when (val discriminator = read()) {
            ...
            0x01 -> {
                ...
                LocalFundingStatus.ConfirmedFundingTx(signedTx.txid, signedTx.txOut.first(), fee, localSigs, shortChannelId)
            }
            0x02 -> {
                val signedTx = readTransaction()
                val fee = readNumber().sat
        ...
        // Note that the channel output is always the first output of the funding transaction, because we use serial_id = 0 in InteractiveTx.kt.
        val fundingInput = OutPoint(localFundingStatus.txId, 0)
        val fundingAmount = when (localFundingStatus) {
            is LocalFundingStatus.ConfirmedFundingTx -> localFundingStatus.txOut.amount
            is LocalFundingStatus.UnconfirmedFundingTx -> localFundingStatus.fundingParams.fundingAmount
        }
        ...
        val localCommit = readLocalCommitWithoutHtlcs(htlcs, remoteFundingPubkey)
