    ...
    companion object {
        fun fromCommitSig(keyManager: KeyManager.ChannelKeys, params: ChannelParams, fundingTxIndex: Long,
            ...
            val signedCommitTx = when (commit.sigOrPartialSig) {
                ...
                is Either.Right -> {
                    val localNonce = keyManager.verificationNonce(fundingTxIndex, localCommitIndex)
            ...
            when (val check = Transactions.checkSpendable(signedCommitTx)) {
                is Try.Failure -> {
                    log.error(check.error) { "remote signature $commit is invalid, localNonce = ${keyManager.verificationNonce(fundingTxIndex, localCommitIndex)}" }
            ...
            val htlcSigs = sortedHtlcTxs.map { it.sign(keyManager.htlcKey.deriveForCommitment(localPerCommitmentPoint)) }
...
data class Commitment(
    ...
    fun sendCommit(
        ...
        val sig = remoteCommitTx.sign(channelKeys.fundingKey(fundingTxIndex))
        val partialSig = when (remoteCommitTx.input) {
        ...
        // we sign our peer's HTLC txs with SIGHASH_SINGLE || SIGHASH_ANYONECANPAY
        val htlcSigs = sortedHtlcTxs.map { it.sign(channelKeys.htlcKey.deriveForCommitment(remoteNextPerCommitmentPoint), SigHash.SIGHASH_SINGLE or SigHash.SIGHASH_ANYONECANPAY) }
                ...
                when (remoteNonce) {
                    null -> {
                        val alternativeSigs = alternativeRemoteCommitTxs.map {
                            val alternativeSig = it.second.sign(channelKeys.fundingKey(fundingTxIndex))
