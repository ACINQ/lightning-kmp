...
data class LocalCommit(val index: Long, val spec: CommitmentSpec, val txId: TxId, val remoteSig: ChannelSpendSignature, val htlcRemoteSigs: List<ByteVector64>) {
    companion object {
        fun fromCommitSig(
            ...
            if (!localCommitTx.checkRemoteSig(fundingKey.publicKey(), remoteFundingPubKey, ChannelSpendSignature.IndividualSignature(commit.signature))) {
            ...
            return Either.Right(LocalCommit(localCommitIndex, spec, localCommitTx.tx.txid, ChannelSpendSignature.IndividualSignature(commit.signature), commit.htlcSignatures))
        }
    }
}
...
sealed class LocalFundingStatus {
    ...
    data class ConfirmedFundingTx(override val txId: TxId, val txOut: TxOut, override val fee: Satoshi, val localSigs: TxSignatures, val shortChannelId: ShortChannelId) : LocalFundingStatus() {
...
data class FullCommitment(val channelParams: ChannelParams, val changes: CommitmentChanges, val commitment: Commitment) {
    ...
    fun fullySignedCommitTx(channelKeys: ChannelKeys): Transaction {
        val fundingKey = channelKeys.fundingKey(fundingTxIndex)
        val commitKeys = localKeys(channelKeys)
        val (unsignedCommitTx, _) = Commitments.makeLocalTxs(channelParams, localCommitParams, commitKeys, localCommit.index, fundingKey, remoteFundingPubkey, commitInput(channelKeys), commitmentFormat, localCommit.spec)
    ...
    fun unsignedHtlcTxs(channelKeys: ChannelKeys): List<Pair<HtlcTx, ByteVector64>> {
        val fundingKey = channelKeys.fundingKey(fundingTxIndex)
        val commitKeys = localKeys(channelKeys)
        val (_, htlcTxs) = Commitments.makeLocalTxs(channelParams, localCommitParams, commitKeys, localCommit.index, fundingKey, remoteFundingPubkey, commitInput(channelKeys), commitmentFormat, localCommit.spec)
...
data class Commitments(
    ...
    fun sendCommit(channelKeys: ChannelKeys, log: MDCLogger): Either<ChannelException, Pair<Commitments, List<CommitSig>>> {
    ...
    fun receiveCommit(commits: List<CommitSig>, channelKeys: ChannelKeys, log: MDCLogger): Either<ChannelException, Pair<Commitments, RevokeAndAck>> {
