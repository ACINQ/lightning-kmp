...
class ClosingTestsCommon : EclairTestSuite() {
    ...
    fun `recv BITCOIN_FUNDING_SPENT (local commit)`() {
        ...
        val (alice1, _) = aliceClosing.process(ChannelEvent.WatchReceived(WatchEventSpent(ByteVector32.Zeroes, BITCOIN_FUNDING_SPENT, aliceCommitTx)))
    ...
    fun `recv BITCOIN_OUTPUT_SPENT`() {
        ...
        val (alice1, _) = aliceClosing.process(ChannelEvent.WatchReceived(WatchEventSpent(ByteVector32.Zeroes, BITCOIN_OUTPUT_SPENT, claimHtlcSuccessFromCommitTx)))
        assertEquals(aliceClosing, alice1)
        ...
        val (alice2, _) = alice1.process(ChannelEvent.WatchReceived(WatchEventSpent(ByteVector32.Zeroes, BITCOIN_OUTPUT_SPENT, claimHtlcSuccessTx)))
    ...
    fun `recv BITCOIN_FUNDING_SPENT (remote commit)`() {
        ...
        assertNotNull(remoteCommitPublished.claimMainDelayedOutputTx)
    ...
    fun `recv BITCOIN_TX_CONFIRMED (remote commit)`() {
        ...
        // bob publishes his last current commit tx, the one it had when entering NEGOTIATING state
        val bobCommitTx = bobCommitTxes.last().commitTx.tx
        assertEquals(4, bobCommitTx.txOut.size) // two main outputs
        ...
        val (alice2, _) = alice1.process(
            ChannelEvent.WatchReceived(
                WatchEventConfirmed(
                    ByteVector32.Zeroes,
                    BITCOIN_TX_CONFIRMED(remoteCommitPublished.claimMainDelayedOutputTx!!),
                    0, 0, remoteCommitPublished.claimMainDelayedOutputTx!!
                )
            )
        )
    ...
    fun `recv BITCOIN_TX_CONFIRMED (remote commit with multiple htlcs for the same payment)`() {
        ...
        assertEquals(7, bobCommitTx.txOut.size)
        ...
        val (alice9, _) = alice8.process(
            ChannelEvent.WatchReceived(
                WatchEventConfirmed(
                    ByteVector32.Zeroes,
                    BITCOIN_TX_CONFIRMED(remoteCommitPublished.claimMainDelayedOutputTx!!),
                    204, 1, remoteCommitPublished.claimMainDelayedOutputTx!!
                )
            )
        )
    ...
    fun `recv BITCOIN_TX_CONFIRMED (remote commit) followed by CMD_FULFILL_HTLC`() {
        ...
        assertEquals(5, bobCommitTx.txOut.size) // two main outputs + 1 HTLC
        ...
        val claimMainTx = (alice5 as Closing).remoteCommitPublished?.claimMainDelayedOutputTx
    ...
    fun `recv BITCOIN_TX_CONFIRMED (next remote commit)`() {
        ...
        val (alice7, _) = alice6.process(
            ChannelEvent.WatchReceived(
                WatchEventConfirmed(
                    ...
                    201,
                    0,
                    remoteCommitPublished.claimHtlcTimeoutTxs.first()
                )
            )
        )
        ...
        val (alice10, _) = alice9.process(ChannelEvent.WatchReceived(WatchEventConfirmed(ByteVector32.Zeroes, BITCOIN_TX_CONFIRMED(remoteCommitPublished.claimMainDelayedOutputTx!!), 203, 2, remoteCommitPublished.claimMainDelayedOutputTx!!)))
    ...
    fun `recv BITCOIN_TX_CONFIRMED (next remote commit) followed by CMD_FULFILL_HTLC`() {
        ...
        assertEquals(6, bobCommitTx.txOut.size)  // two main outputs + 2 HTLCs
        ...
        assertNotNull(claimHtlcSuccessTx)
        Transaction.correctlySpends(claimHtlcSuccessTx, bobCommitTx, ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS)
        assertEquals(ChannelAction.Blockchain.PublishTx(alice5.nextRemoteCommitPublished?.claimMainDelayedOutputTx!!), publishTxs[0])
        assertEquals(ChannelAction.Blockchain.PublishTx(claimHtlcSuccessTx), publishTxs[1])
    ...
    fun `recv BITCOIN_TX_CONFIRMED (future remote commit)`() {
        ...
        val (alice8, aliceActions8) = Offline(alice0).process(ChannelEvent.Connected(localInit, remoteInit))
        assertTrue { alice8 is Syncing }
        val channelReestablishA = (aliceActions8[0] as ChannelAction.Message.Send).message as ChannelReestablish
        val (bob8, bobActions8) = bob7.process(ChannelEvent.Connected(remoteInit, localInit))
        assertTrue { bob8 is Syncing }
        val channelReestablishB = (bobActions8[0] as ChannelAction.Message.Send).message as ChannelReestablish

        // peers exchange channel_reestablish messages
        val (alice9, aliceActions9) = alice8.process(ChannelEvent.MessageReceived(channelReestablishB))
        val (bob9, _) = bob8.process(ChannelEvent.MessageReceived(channelReestablishA))
    ...
    fun `recv BITCOIN_FUNDING_SPENT (one revoked tx)`() {
        ...
        assertEquals(5, bobRevokedTx.txOut.size)

        val (alice1, aliceActions1) = alice.process(ChannelEvent.WatchReceived(WatchEventSpent(ByteVector32.Zeroes, BITCOIN_FUNDING_SPENT, bobRevokedTx)))
    ...
    fun `recv BITCOIN_FUNDING_SPENT (multiple revoked tx)`() {
        ...
        val (alice1, aliceActions1) = alice.process(ChannelEvent.WatchReceived(WatchEventSpent(ByteVector32.Zeroes, BITCOIN_FUNDING_SPENT, bobCommitTxes.first().commitTx.tx)))
        assertTrue { alice1 is Closing }; alice1 as Closing
        ...
        val (alice2, aliceActions2) = alice1.process(ChannelEvent.WatchReceived(WatchEventSpent(ByteVector32.Zeroes, BITCOIN_FUNDING_SPENT, bobCommitTxes[1].commitTx.tx)))
        ...
        val (alice3, aliceActions3) = alice2.process(ChannelEvent.WatchReceived(WatchEventSpent(ByteVector32.Zeroes, BITCOIN_FUNDING_SPENT, bobCommitTxes[1].commitTx.tx)))
        ...
        assertEquals(bobCommitTxes[1].commitTx.tx.txid, aliceActions3.findWatches<WatchConfirmed>()[0].txId)
