...
class PaymentRequestTestsCommon {
    ...
    fun `On mainnet, please send 0,00967878534 BTC for a list of items within one week, amount in pico BTC`() {
        ...
        assertEquals(pr.routingInfo, listOf(PaymentRequest.TaggedField.RoutingInfo(listOf(PaymentRequest.TaggedField.ExtraHop(PublicKey(ByteVector("03d06758583bb5154774a6eb221b1276c9e82d65bbaceca806d90e20c108f4b1c7")), ShortChannelId("589390x3312x1"), 1000.msat, 2500, CltvExpiryDelta(40))))))
        val check = pr.sign(priv).write()
        assertEquals(ref, check)
    }
    ...
    @Test
    fun `payment secret`() {
        val features = Features(setOf(ActivatedFeature(Feature.VariableLengthOnion, FeatureSupport.Optional), ActivatedFeature(Feature.PaymentSecret, FeatureSupport.Optional), ActivatedFeature(Feature.BasicMultiPartPayment, FeatureSupport.Optional)))
        ...
        assertFails { PaymentRequest.create(Block.LivenetGenesisBlock.hash, 123.msat, ByteVector32.One, priv, "MPP without secrets", CltvExpiryDelta(18), Features(setOf(ActivatedFeature(Feature.VariableLengthOnion, FeatureSupport.Optional), ActivatedFeature(Feature.BasicMultiPartPayment, FeatureSupport.Optional)))) }
    }
