...
class SpliceTestsCommon : LightningTestSuite() {
    ...
    fun `splice to purchase inbound liquidity`() {
        ...
        val liquidityRequest = LiquidityAds.RequestFunding(200_000.sat, fundingRates.findLease(200_000.sat)!!, LiquidityAds.PaymentDetails.FromChannelBalance)
    ...
    fun `splice to purchase inbound liquidity -- not enough funds`() {
        ...
        run {
            val liquidityRequest = LiquidityAds.RequestFunding(1_000_000.sat, leaseRate, LiquidityAds.PaymentDetails.FromChannelBalance)
        ...
        run {
            val liquidityRequest = LiquidityAds.RequestFunding(1_000_000.sat, leaseRate.copy(rate = leaseRate.rate.copy(leaseFeeBase = 0.sat)), LiquidityAds.PaymentDetails.FromChannelBalance)
            ...
            val aliceStfu = actionsAlice1.findOutgoingMessage<Stfu>()
            val (_, actionsBob2) = bob1.process(ChannelCommand.MessageReceived(aliceStfu))
            actionsBob2.hasOutgoingMessage<SpliceInit>().also { assertEquals(liquidityRequest, it.requestFunding) }
        }
        run {
            // When we don't  have enough funds in our channel balance, fees can be paid via future HTLCs.
            val liquidityRequest = LiquidityAds.RequestFunding(1_000_000.sat, leaseRate, LiquidityAds.PaymentDetails.FromFutureHtlc(listOf(randomBytes32())))
            assertEquals(10_001.sat, liquidityRequest.fees(FeeratePerKw(1000.sat)).total)
            val cmd = ChannelCommand.Commitment.Splice.Request(CompletableDeferred(), null, null, liquidityRequest, FeeratePerKw(1000.sat), listOf())
            val (bob1, actionsBob1) = bob.process(cmd)
            val bobStfu = actionsBob1.findOutgoingMessage<Stfu>()
    ...
    fun `splice to purchase inbound liquidity -- not enough funds but on-the-fly funding`() {
        ...
        val fundingRate = LiquidityAds.FundingLease.Basic(0.sat, 500_000.sat, LiquidityAds.LeaseRate(0, 0.sat, 50))
