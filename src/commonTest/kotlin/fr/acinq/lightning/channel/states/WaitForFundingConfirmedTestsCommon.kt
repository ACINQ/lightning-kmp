...
class WaitForFundingConfirmedTestsCommon : LightningTestSuite() {
    ...
    fun `on reconnection a zero-conf channel needs to register a zero-confirmation watch on funding tx`() {
        ...
        val (alice, bob) = init(ChannelType.SupportedChannelType.AnchorOutputsZeroConfZeroReserve, TestConstants.aliceFundingAmount, TestConstants.bobFundingAmount, TestConstants.pushAmount, bobFeatures = bobFeatures)
        val (bob1, _) = bob.processEx(ChannelEvent.Disconnected)
        assertTrue(bob1 is Offline)
    ...
    companion object {
        fun init(
            ...
            val (alice, bob, fundingCreated) = WaitForFundingCreatedTestsCommon.init(channelType, aliceFundingAmount, bobFundingAmount, pushAmount, aliceFeatures, bobFeatures)
