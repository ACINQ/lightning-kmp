...
class LocalKeyManagerTestsCommon : LightningTestSuite() {
    ...
    fun `generate the same node id from the same seed`() {
        ...
        val keyManager = LocalKeyManager(seed, Bitcoin.Chain.Regtest, TestConstants.aliceSwapInServerXpub)
    ...
    fun `generate different node ids from the same seed on different chains`() {
        val seed = ByteVector("17b086b228025fa8f4416324b6ba2ec36e68570ae2fc3d392520969f2a9d0c1501")
        val keyManager1 = LocalKeyManager(seed, Bitcoin.Chain.Regtest, DeterministicWallet.encode(dummyExtendedPubkey, testnet = true))
        val keyManager2 = LocalKeyManager(seed, Bitcoin.Chain.Mainnet, DeterministicWallet.encode(dummyExtendedPubkey, testnet = false))
    ...
    fun `test vectors -- mainnet + initiator`() {
        ...
        val keyManager = LocalKeyManager(seed, Bitcoin.Chain.Mainnet, DeterministicWallet.encode(dummyExtendedPubkey, testnet = false))
    ...
    fun `compute descriptors to recover swap-in funds`() {
        ...
        val chain = Bitcoin.Chain.Regtest
