...
class PaymentRequestTestsCommon : LightningTestSuite() {
    ...
    fun `handle amounts that are multiples of 1BTC`() {
        ...
        testCases.forEach { (amount, ref) ->
            val invoice = PaymentRequest.read(ref).get()
    ...
    fun `Please make a donation of any amount using payment_hash 0001020304050607080900010203040506070809000102030405060708090102 to me at 03e7156ae33b0a208d0744199163177e909e80176e55d97a2f221ede0f934dd9ad`() {
        ...
        val pr = PaymentRequest.read(ref).get()
    ...
    fun `reject invalid invoices`() {
        ...
        refs.forEach {
            assertTrue(PaymentRequest.read(it).isFailure)
        }
    }
    ...
    fun `ignore unknown tags`() {
        ...
        val pr1 = PaymentRequest.read(serialized).get()
    ...
    fun `filter non-invoice features when parsing invoices`() {
        ...
        val pr = PaymentRequest.read(createInvoiceUnsafe(features = features).write()).get()
    ...
    fun `payment secret`() {
        ...
        val pr1 = PaymentRequest.read(pr.write()).get()
        assertEquals(pr1.paymentSecret, pr.paymentSecret)

        // An invoice without the payment secret feature should be rejected
        assertTrue(PaymentRequest.read("lnbc40n1pw9qjvwpp5qq3w2ln6krepcslqszkrsfzwy49y0407hvks30ec6pu9s07jur3sdpstfshq5n9v9jzucm0d5s8vmm5v5s8qmmnwssyj3p6yqenwdencqzysxqrrss7ju0s4dwx6w8a95a9p2xc5vudl09gjl0w2n02sjrvffde632nxwh2l4w35nqepj4j5njhh4z65wyfc724yj6dn9wajvajfn5j7em6wsq2elakl").isFailure)

        // An invoice that sets the payment secret feature bit must provide a payment secret.
        assertTrue(PaymentRequest.read("lnbc1230p1pwljzn3pp5qyqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqdq52dhk6efqd9h8vmmfvdjs9qypqsqylvwhf7xlpy6xpecsnpcjjuuslmzzgeyv90mh7k7vs88k2dkxgrkt75qyfjv5ckygw206re7spga5zfd4agtdvtktxh5pkjzhn9dq2cqz9upw7").isFailure)
