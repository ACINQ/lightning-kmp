...
class PaymentPacketTestsCommon : LightningTestSuite() {
    ...
    fun `send a trampoline payment to blinded recipient`() {
        ...
        val blinding = innerC.records.get<OnionPaymentPayloadTlv.BlindingPoint>()?.publicKey
        assertNotNull(blinding)
        assertEquals(blindedRoute.blindingKey, blinding)
        val (encryptedPayload, nextBlinding) = RouteBlinding.decryptPayload(privC, blinding, encryptedData).right!!
        ...
        val onionD = run {
            val payloadD = PaymentOnion.FinalPayload.Standard(
                TlvStream(
                    ...
                    OnionPaymentPayloadTlv.BlindingPoint(nextBlinding),
    ...
    fun `send a trampoline payment to blinded recipient -- reference test vector`() {
        ...
        val trampolineOnion = run {
            val carol = PublicKey.fromHex("027f31ebc5462c1fdce1b737ecff52d37d75dea43ce11c74d25aa297165faa2007")
            val dave = PublicKey.fromHex("032c0b7cf95324a07d05398b240174dc0c2be444d96b159aa6c7f7b1e668680991")
            assertEquals(EncodedNodeId(dave), path.route.route.introductionNodeId)
            assertEquals(PublicKey.fromHex("020e2dbadcc2005e859819ddebbe88a834ae8a6d2b049233c07335f15cd1dc5f22"), path.route.nodeId)
            val payloadEve = PaymentOnion.FinalPayload.Blinded.create(150_000_000.msat, CltvExpiry(800_000), path.route.route.encryptedPayloads.last(), blinding = null)
            assertContentEquals(
            ...
            val payloadDave = PaymentOnion.BlindedChannelRelayPayload.create(path.route.route.encryptedPayloads.first(), path.route.route.blindingKey)
            assertContentEquals(
        ...
        val onionForEve = run {
            ...
            assertNotNull(encryptedData)
            val blinding = innerPayload.records.get<OnionPaymentPayloadTlv.BlindingPoint>()?.publicKey
            assertNotNull(blinding)
            val (encryptedPayload, nextBlinding) = RouteBlinding.decryptPayload(davePriv, blinding, encryptedData).right!!
            val decryptedPayload = RouteBlindingEncryptedData.read(encryptedPayload.toByteArray()).right!!
