...
class IncomingPaymentHandlerTestsCommon : LightningTestSuite() {
    ...
    fun `receive multipart payment with single HTLC`() = runSuspendTest {
        ...
        val result = paymentHandler.process(add, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
    ...
    fun `receive multipart payment after disconnection`() = runSuspendTest {
        ...
        run {
            val result = paymentHandler.process(add1, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
            assertIs<IncomingPaymentHandler.ProcessAddResult.Pending>(result)
            assertNull(result.incomingPayment.received)
            assertTrue(result.actions.isEmpty())
        }
    ...
    fun `receive will_add_htlc`() = runSuspendTest {
        ...
        val result = paymentHandler.process(willAddHtlc, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
        assertIs<IncomingPaymentHandler.ProcessAddResult.Pending>(result)
    ...
    fun `receive multipart payment with a mix of HTLC and will_add_htlc`() = runSuspendTest {
        ...
        run {
            ...
            val result = paymentHandler.process(htlc, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
            assertIs<IncomingPaymentHandler.ProcessAddResult.Pending>(result)
            assertTrue(result.actions.isEmpty())
        }
    ...
    fun `receive multipart payment with a mix of HTLC and will_add_htlc -- too many parts`() = runSuspendTest {
        val channelId = randomBytes32()
        val paymentHandler = IncomingPaymentHandler(TestConstants.Bob.nodeParams.copy(maxAcceptedHtlcs = 5), InMemoryPaymentsDb(), TestConstants.fundingRates)
        paymentHandler.nodeParams.liquidityPolicy.emit(LiquidityPolicy.Auto(inboundLiquidityTarget = null, maxAbsoluteFee = 10_000.sat, maxRelativeFeeBasisPoints = 1000, skipAbsoluteFeeCheck = false))
        ...
        paymentHandler.process(htlc, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw).also { result ->
            assertIs<IncomingPaymentHandler.ProcessAddResult.Pending>(result)
            assertTrue(result.actions.isEmpty())
        }
        ...
        willAddHtlcs.take(4).forEach {
            val result = paymentHandler.process(it, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
            assertIs<IncomingPaymentHandler.ProcessAddResult.Pending>(result)
            assertTrue(result.actions.isEmpty())
        }

        // Alice sends the last will_add_htlc: there are too many parts, so Bob rejects the payment.
        val result = paymentHandler.process(willAddHtlcs.last(), Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
        assertIs<IncomingPaymentHandler.ProcessAddResult.Rejected>(result)
    ...
    fun `receive will_add_htlc added to fee credit`() = runSuspendTest {
        ...
        testCases.forEach { (currentFeeCredit, failure) ->
            ...
            val result = paymentHandler.process(willAddHtlc, feeCreditFeatures, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw, currentFeeCredit)
            when (failure) {
                null -> {
                    assertIs<IncomingPaymentHandler.ProcessAddResult.Accepted>(result)
    ...
    fun `receive multipart payment with a mix of HTLC and will_add_htlc added to fee credit`() = runSuspendTest {
        ...
        run {
            val htlc = makeUpdateAddHtlc(0, channelId, paymentHandler, incomingPayment.paymentHash, makeMppPayload(amount1, totalAmount, paymentSecret))
            val result = paymentHandler.process(htlc, feeCreditFeatures, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw, currentFeeCredit = 0.msat)
            assertIs<IncomingPaymentHandler.ProcessAddResult.Pending>(result)
            assertTrue(result.actions.isEmpty())
        }
        ...
        run {
            val willAddHtlc = makeWillAddHtlc(paymentHandler, incomingPayment.paymentHash, makeMppPayload(amount2, totalAmount, paymentSecret))
            val result = paymentHandler.process(willAddHtlc, feeCreditFeatures, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw, currentFeeCredit = 0.msat)
            assertIs<IncomingPaymentHandler.ProcessAddResult.Accepted>(result)
            val (expectedActions, expectedReceivedWith) = setOf(
                // @formatter:off
                WrappedChannelCommand(channelId, ChannelCommand.Htlc.Settlement.Fulfill(0, incomingPayment.preimage, commit = true)) to IncomingPayment.ReceivedWith.LightningPayment(amount1, channelId, 0, fundingFee = null),
    ...
    fun `receive will_add_htlc with enough fee credit`() = runSuspendTest {
        ...
        assertIs<AddLiquidityForIncomingPayment>(addLiquidity)
        assertEquals(totalAmount, addLiquidity.paymentAmount)
        assertEquals(100_000.sat, addLiquidity.requestedAmount)
        // We don't update the payments DB: we're waiting to receive HTLCs after the open/splice.
        assertNull(paymentHandler.db.getIncomingPayment(incomingPayment.paymentHash)?.received)
    }
    ...
    fun `receive will_add_htlc larger than fee credit threshold`() = runSuspendTest {
        ...
        val (paymentHandler, incomingPayment, paymentSecret) = createFeeCreditFixture(totalAmount, LiquidityPolicy.Auto(100_000.sat, 5000.sat, 1000, skipAbsoluteFeeCheck = false))
        val willAddHtlc = makeWillAddHtlc(paymentHandler, incomingPayment.paymentHash, makeMppPayload(totalAmount, totalAmount, paymentSecret))
        val result = paymentHandler.process(willAddHtlc, feeCreditFeatures, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw, currentFeeCredit = 100.msat)
        assertIs<IncomingPaymentHandler.ProcessAddResult.Pending>(result)
        assertEquals(1, result.actions.size)
        val addLiquidity = result.actions.first()
        assertIs<AddLiquidityForIncomingPayment>(addLiquidity)
        assertEquals(totalAmount, addLiquidity.paymentAmount)
        assertEquals(120_000.sat, addLiquidity.requestedAmount)
        // We don't update the payments DB: we're waiting to receive HTLCs after the open/splice.
        assertNull(paymentHandler.db.getIncomingPayment(incomingPayment.paymentHash)?.received)
    }
    ...
    fun `receive multipart payment with amount greater than total amount`() = runSuspendTest {
        ...
        // - Bob now accepts the MPP set
        run {
            val result = paymentHandler.process(add3, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
            assertIs<IncomingPaymentHandler.ProcessAddResult.Accepted>(result)
            val expected = setOf(
    ...
    fun `receive multipart payment with greater expiry`() = runSuspendTest {
        ...
        val result = paymentHandler.process(addGreaterExpiry, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
        assertIs<IncomingPaymentHandler.ProcessAddResult.Accepted>(result)
    ...
    fun `reprocess duplicate htlcs`() = runSuspendTest {
        ...
        val result1 = paymentHandler.process(add1, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
        ...
        val result1b = paymentHandler.process(add1, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
        ...
        val result2 = paymentHandler.process(add2, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
        ...
        val result2b = paymentHandler.process(add2, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
    ...
    fun `reprocess failed htlcs`() = runSuspendTest {
        ...
        val result1 = paymentHandler.process(add, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
        assertIs<IncomingPaymentHandler.ProcessAddResult.Pending>(result1)
        assertTrue(result1.actions.isEmpty())
        ...
        val result2 = paymentHandler.process(add, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
        ...
        val result3 = paymentHandler.process(add, Features.empty, currentBlockHeight, TestConstants.feeratePerKw)
    ...
    fun `fulfill locally replayed htlcs`() = runSuspendTest {
        ...
        run {
            val result1 = paymentHandler.process(htlc1, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
            assertIs<IncomingPaymentHandler.ProcessAddResult.Pending>(result1)
            assertTrue(result1.actions.isEmpty())

            val result2 = paymentHandler.process(htlc2, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
            assertIs<IncomingPaymentHandler.ProcessAddResult.Accepted>(result2)
        ...
        run {
            val result = paymentHandler.process(htlc1, Features.empty, TestConstants.defaultBlockHeight, TestConstants.feeratePerKw)
            assertIs<IncomingPaymentHandler.ProcessAddResult.Accepted>(result)
    ...
    companion object {
        ...
        private suspend fun createFeeCreditFixture(invoiceAmount: MilliSatoshi, policy: LiquidityPolicy): Triple<IncomingPaymentHandler, IncomingPayment, ByteVector32> {
            ...
            val paymentHandler = IncomingPaymentHandler(nodeParams, InMemoryPaymentsDb(), TestConstants.fundingRates)
            val (incomingPayment, paymentSecret) = makeIncomingPayment(paymentHandler, invoiceAmount)
            return Triple(paymentHandler, incomingPayment, paymentSecret)
        }
    }
