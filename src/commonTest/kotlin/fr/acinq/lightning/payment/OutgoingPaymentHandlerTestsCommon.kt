...
class OutgoingPaymentHandlerTestsCommon : LightningTestSuite() {
    ...
    @Test
    fun `several large blinded paths`() = runSuspendTest {
        val (alice, _) = TestsHelper.reachNormal()
        ...
        val outgoingPaymentHandler = OutgoingPaymentHandler(alice.staticParams.nodeParams, defaultWalletParams, InMemoryPaymentsDb())
        val payInvoice = PayInvoice(UUID.randomUUID(), 20_000.msat, LightningOutgoingPayment.Details.Blinded(invoice, payerKey))
        val (trampolineAmount, trampolineExpiry, trampolinePacket) = outgoingPaymentHandler.createTrampolinePayload(payInvoice, defaultWalletParams.trampolineFees.first(), 851851)
        ...
        val childPayments = outgoingPaymentHandler.createChildPayments(payInvoice, listOf(RouteCalculation.Route(24080.msat, alice.state)), trampolinePayload)
        println(childPayments)
    }
