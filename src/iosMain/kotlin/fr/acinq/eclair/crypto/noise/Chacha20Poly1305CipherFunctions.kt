...
actual object Chacha20Poly1305CipherFunctions : CipherFunctions {
    ...
    override fun encrypt(k: ByteArray, n: Long, ad: ByteArray, plaintext: ByteArray): ByteArray {
        ...
        var kResult: ByteArray? = null
        if (!useNative || testNative) {
            val (ciphertext, mac) = ChaCha20Poly1305.encrypt(
                k, nonce(n), plaintext, ad
            )
            kResult = ciphertext + mac
        }
        if (useNative) {
            ...
            val nResult = ciphertextAndMac.toByteArray()

            if (testNative) {
                require(kResult == nResult)
            }
            return nResult
        } else {
            return kResult!!
    ...
    override fun decrypt(k: ByteArray, n: Long, ad: ByteArray, ciphertextAndMac: ByteArray): ByteArray {
        ...
        if (!useNative || testNative) {
            ...
            kResult = ChaCha20Poly1305.decrypt(
                k, nonce(n), ciphertext, ad, mac
            )
        }
        if (useNative) {
            ...
            val nResult = plaintext.toByteArray()
