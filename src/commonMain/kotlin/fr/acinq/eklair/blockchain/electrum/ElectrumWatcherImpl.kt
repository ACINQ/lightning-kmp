...
import fr.acinq.eklair.blockchain.electrum.ElectrumWatcherImpl.Companion.logger
...
class ElectrumWatcherImpl(val client: ElectrumClient, val scope: CoroutineScope): ElectrumWatcher, CoroutineScope by scope {
    ...
    override fun send(message: WatcherEvent) {
        launch { eventChannel.send(message) }
    }

    override fun watch(watch: Watch) {
...
//        client.sendMessage(UnsubscribeListener(electrumMessageChannel))// TODO?
    ...
    companion object {
        // TODO inject
        val logger = LoggerFactory(simplePrintFrontend).newLogger(ElectrumWatcherImpl::class)
    }
}