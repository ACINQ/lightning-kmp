...
object Helpers {
    ...
    object Funding {
        ...
        fun makeCommitTxsWithoutHtlcs(
            ...
            val remoteCommitTx = Commitments.makeRemoteTxs(channelKeys, commitmentIndex, localParams, remoteParams, commitmentInput, remotePerCommitmentPoint, remoteSpec).first
    ...
    object Closing {
        ...
        fun makeClosingTx(
            ...
            val localClosingSig = Transactions.sign(closingTx, channelKeys.fundingPrivateKey)
        ...
        fun checkClosingSignature(
            ...
            return if (checkClosingDustAmounts(closingTx)) {
                val signedClosingTx = Transactions.addSigs(closingTx, channelKeys.fundingPubKey, commitment.params.remoteParams.fundingPubKey, closingSigned.signature, remoteClosingSig)
        ...
        fun LoggingContext.claimCurrentLocalCommitTxOutputs(channelKeys: KeyManager.ChannelKeys, commitment: FullCommitment, tx: Transaction, feerates: OnChainFeerates): LocalCommitPublished {
            ...
            val localDelayedPubkey = channelKeys.delayedPaymentBasepoint.derive(localPerCommitmentPoint)
            ...
            val mainDelayedTx = generateTx("main-delayed-output") {
                ...
                val sig = Transactions.sign(it, channelKeys.delayedPaymentKey.derive(localPerCommitmentPoint), SigHash.SIGHASH_ALL)
                        ...
                        generateTx("claim-htlc-success") {
                            ...
                            when (val preimage = preimages.firstOrNull { r -> r.sha256() == htlc.add.paymentHash }) {
                                ...
                                // incoming htlc for which we have the preimage: we can spend it directly
                                    ...
                                    val sig = Transactions.sign(claimHtlcTx, channelKeys.htlcKey.derive(remoteCommit.remotePerCommitmentPoint), SigHash.SIGHASH_ALL)
        ...
        fun LoggingContext.claimRevokedRemoteCommitTxOutputs(channelKeys: KeyManager.ChannelKeys, params: ChannelParams, remotePerCommitmentSecret: PrivateKey, commitTx: Transaction, feerates: OnChainFeerates): RevokedCommitPublished {
            ...
            val mainPenaltyTx = generateTx("main-penalty") {
                ...
                val sig = Transactions.sign(it, channelKeys.revocationKey.deriveRevocation(remotePerCommitmentSecret))
                Transactions.addSigs(it, sig)
            }
            ...
            val htlcPenaltyTxs = revokedCommitPublished.commitTx.txOut.mapIndexedNotNull { outputIndex, txOut ->
                htlcsRedeemScripts[txOut.publicKeyScript]?.let { redeemScript ->
                    generateTx("htlc-penalty") {
                        ...
                        val sig = Transactions.sign(htlcPenaltyTx, channelKeys.revocationKey.deriveRevocation(revokedCommitPublished.remotePerCommitmentSecret))
            ...
            if (isHtlcTx) {
                ...
                val penaltyTxs = Transactions.makeClaimDelayedOutputPenaltyTxs(
                    ...
                    generateTx("claim-htlc-delayed-penalty") {
                        ...
                        val sig = Transactions.sign(it, channelKeys.revocationKey.deriveRevocation(revokedCommitPublished.remotePerCommitmentSecret))
