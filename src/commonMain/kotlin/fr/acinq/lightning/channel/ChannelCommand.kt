...
sealed class ChannelCommand {
    ...
    data class BumpFundingFee(val targetFeerate: FeeratePerKw, val fundingAmount: Satoshi, val utxos: List<WalletState.Utxo>, val lockTime: Long) : ChannelCommand()
    ...
    sealed class Splice {
        data class Request(val replyTo: CompletableDeferred<Response>, val spliceIn: SpliceIn?, val spliceOut: SpliceOut?, val feerate: FeeratePerKw, val origins: List<Origin.PayToOpenOrigin> = emptyList()) : ChannelCommand() {
            ...
            data class SpliceIn(val utxos: List<WalletState.Utxo>, val pushAmount: MilliSatoshi = 0.msat)
