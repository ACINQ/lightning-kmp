...
object LiquidityAds {
    ...
    data class WillFundRates(val fundingRates: List<FundingLease>, val paymentTypes: Set<PaymentType>) {
        fun validateRequest(nodeKey: PrivateKey, fundingScript: ByteVector, fundingFeerate: FeeratePerKw, request: RequestFunding): WillFundLease? {
    ...
    data class RequestFunding(val requestedAmount: Satoshi, val fundingLease: FundingLease, val paymentDetails: PaymentDetails) {
        ...
        companion object {
            fun chooseLease(requestedAmount: Satoshi, paymentDetails: PaymentDetails, rates: WillFundRates): RequestFunding? = when {
                rates.paymentTypes.contains(paymentDetails.paymentType) -> rates.findLease(requestedAmount)?.let { RequestFunding(requestedAmount, it, paymentDetails) }
                else -> null
            }
