...
class IncomingPaymentHandler(val nodeParams: NodeParams, val db: PaymentsDb, private val remoteFundingRates: LiquidityAds.WillFundRates) {
    ...
    suspend fun process(htlc: UpdateAddHtlc, remoteFeatures: Features, currentBlockHeight: Int, currentFeerate: FeeratePerKw, currentFeeCredit: MilliSatoshi = 0.msat): ProcessAddResult {
        return process(Either.Right(htlc), remoteFeatures, currentBlockHeight, currentFeerate, currentFeeCredit)
    }

    /** Process an incoming on-the-fly funding request. */
    suspend fun process(htlc: WillAddHtlc, remoteFeatures: Features, currentBlockHeight: Int, currentFeerate: FeeratePerKw, currentFeeCredit: MilliSatoshi = 0.msat): ProcessAddResult {
        return process(Either.Left(htlc), remoteFeatures, currentBlockHeight, currentFeerate, currentFeeCredit)
    }

    private suspend fun process(htlc: Either<WillAddHtlc, UpdateAddHtlc>, remoteFeatures: Features, currentBlockHeight: Int, currentFeerate: FeeratePerKw, currentFeeCredit: MilliSatoshi): ProcessAddResult {
        ...
        return when (val res = toPaymentPart(privateKey, htlc)) {
            ...
            is Either.Right -> processPaymentPart(res.value, remoteFeatures, currentBlockHeight, currentFeerate, currentFeeCredit)
        }
    }

    /** Main payment processing, that handles payment parts. */
    private suspend fun processPaymentPart(paymentPart: PaymentPart, remoteFeatures: Features, currentBlockHeight: Int, currentFeerate: FeeratePerKw, currentFeeCredit: MilliSatoshi): ProcessAddResult {
                    ...
                    return when {
                        ...
                        payment.amountReceived + payment.fundingFee < payment.totalAmount -> {
                            ...
                            when {
                                ...
                                willAddHtlcParts.isNotEmpty() -> when (val result = validateOnTheFlyFundingRate(willAddHtlcParts.map { it.amount }.sum(), remoteFeatures, currentFeerate)) {
                                    is Either.Left -> {
                                    ...
                                    is Either.Right -> {
                                        ...
                                        when {
                                            addToFeeCredit -> {
                                                ...
                                                val fees = fundingRate.fees(currentFeerate, requestedAmount, requestedAmount).total.toMilliSatoshi()
                                                when (val rejected = nodeParams.liquidityPolicy.value.maybeReject(requestedAmount.toMilliSatoshi(), fees, LiquidityEvents.Source.OffChainPayment, logger)) {
                                                    is LiquidityEvents.Rejected -> {
                                                        ...
                                                        val actions = listOf(AddLiquidityForIncomingPayment(payment.amountReceived, requestedAmount, fundingRate, incomingPayment.preimage))
    ...
    private fun validateOnTheFlyFundingRate(willAddHtlcAmount: MilliSatoshi, remoteFeatures: Features, currentFeerate: FeeratePerKw): Either<LiquidityEvents.Rejected, Pair<Satoshi, LiquidityAds.FundingRate>> {
