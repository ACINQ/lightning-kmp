                    ...
                    is PeerStorageRetrieval -> {
                        val backup: DeserializationResult? =
                            PersistedChannelState
                                .from(nodeParams.nodePrivateKey, msg.ecd)
                        ...
                        when {
                            backup is DeserializationResult.UnknownVersion -> {
                                logger.warning { "peer sent a storage retrieval with a backup generated by a more recent of phoenix: version=${backup.version}." }
                            ...
                            backup is DeserializationResult.Success -> {
                                val local: ChannelState? = _channels[backup.state.channelId]
                                when {
                                    local == null -> {
                                        logger.warning { "recovering channel from peer backup" }
                                        recoverChannel(backup.state)
                                    }
                                    local is Syncing && local.state is ChannelStateWithCommitments && backup.state is ChannelStateWithCommitments && backup.state.commitments.isMoreRecent(local.state.commitments) -> {
