        ...
        return if (amount < localDustLimit) {
            ...
            when (isTaprootChannel) {
                true -> {
                    ...
                    val tree = ScriptTree.Leaf(0, Scripts.Taproot.toDelayScript(localDelayedPaymentPubkey, toLocalDelay))
                    val tx = Transaction(
                        version = 2,
...
fun makeHtlcDelayedTx(htlcTx: Transaction,
    ...
    return when(isTaprootChannel) {
      true -> {
          val htlcTxTree = ScriptTree.Leaf (0, Scripts.Taproot.toDelayScript(localDelayedPaymentPubkey, toLocalDelay))
    ...
    fun makeClaimDelayedOutputPenaltyTxs(
        ...
        val (redeemScript, pubkeyScript, scripTree) = when (isTaprootChannel) {
            true -> {
                val tree = ScriptTree.Leaf(0, Taproot.toDelayScript(localDelayedPaymentPubkey, toLocalDelay))
                Triple(
