...
data class ProcessFail(val fail: UpdateFailHtlc, val origin: Origin?) : ChannelAction()
data class ProcessFailMalformed(val fail: UpdateFailMalformedHtlc, val origin: Origin?) : ChannelAction()
data class ProcessFulfill(val fulfill: UpdateFulfillHtlc, val origin: Origin?) : ChannelAction()
                ...
                when (event.message) {
                    ...
                    is UpdateFulfillHtlc -> {
                        ...
                        when (val result = commitments.receiveFulfill(event.message)) {
                            is Try.Failure -> Pair(this, listOf(HandleError(result.error)))
                            is Try.Success -> Pair(this.copy(commitments = result.result.first), listOf(ProcessFulfill(event.message, commitments.originChannels[event.message.id])))
                        }
                    }
                    is UpdateFailHtlc -> {
